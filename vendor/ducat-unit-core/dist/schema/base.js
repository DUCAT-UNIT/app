...
export const zod = z;
export const char = num.min(0).max(0xFF);
export const short = num.min(0).max(0xFFFF);
export const int = num.int().min(-0x80000000).max(0x7FFFFFFF);
export const uint = num.min(0).max(0xFFFFFFFF);
export const long = z.int();
export const ulong = long.nonnegative();
export const float = z.float32();
export const double = z.float64();
export const float2 = float.refine((e) => {
    const parts = String(e).split('.').at(1);
    return parts !== undefined && parts.length <= 2;
});
export const stamp = uint.min(500_000_000);
