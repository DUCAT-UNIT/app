/**
 * @fileoverview Normalized transaction/context type definitions for protocol parsing.
 */
...
export interface ProtoTxOutput {
  script_pk : string
  // 'unknown' is used for scriptPubKeys whose lock type is not
  // recognized by get_lock_script_type. Parsing such an output no
  // longer throws (Codex finding #7) — the output is preserved with
  // type 'unknown' so a single unrecognized vout cannot DoS parsing
  // of the whole transaction. Vault-scanning consumers filter by the
  // specific types they care about (e.g. 'opreturn') and skip these.
