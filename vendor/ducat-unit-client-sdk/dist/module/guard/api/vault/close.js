...
import { validate_vault_close_request } from '../../../../module/vault/lib/validate.js';
import * as SCHEMA from '../../../../schema/index.js';
import * as SHARED from '@ducat-unit/core/schema';
export function close_vault_api(client) {
    return (request) => {
        validate_vault_close_request(request);
        const msg = { data: request, topic: TOPICS.GUARDIAN.VAULT_CLOSE };
        ...
        emit_info(observe, 'guardian.request.vault.close.start', 'created guardian vault close request', { contract_id: request.contract_id });
        sub.on('message', (message) => {
            switch (message.type) {
                ...
                case 'reject': {
                    const parsed = SHARED.base.str.parse(message.data);
                    emit_warn(observe, 'guardian.request.vault.close.reject', 'guardian vault close request rejected', {
                        reason: parsed
                    });
                ...
                case 'result': {
                    const schema = SCHEMA.module.guard.response.vault_close;
                    const parsed = schema.parse(message.data);
                    emit_debug(observe, 'guardian.request.vault.close.result', {
