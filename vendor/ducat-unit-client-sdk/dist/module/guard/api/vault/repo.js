...
import { validate_vault_repo_request } from '../../../../module/vault/lib/validate.js';
import * as SCHEMA from '../../../../schema/index.js';
import * as SHARED from '@ducat-unit/core/schema';
export function repo_vault_api(client) {
    return (request) => {
        validate_vault_repo_request(request);
        const msg = { data: request, topic: TOPICS.GUARDIAN.VAULT_REPO };
        ...
        emit_info(observe, 'guardian.request.vault.repo.start', 'created guardian vault repo request', { contract_id: request.contract_id });
        sub.on('message', (message) => {
            switch (message.type) {
                ...
                case 'reject': {
                    const parsed = SHARED.base.str.parse(message.data);
                    emit_warn(observe, 'guardian.request.vault.repo.reject', 'guardian vault repo request rejected', {
                        reason: parsed
                    });
                ...
                case 'result': {
                    const schema = SCHEMA.module.guard.response.vault_repo;
                    const parsed = schema.parse(message.data);
                    emit_debug(observe, 'guardian.request.vault.repo.result', {
