...
import { validate_vault_borrow_request } from '../../../../module/vault/lib/validate.js';
import * as SCHEMA from '../../../../schema/index.js';
import * as SHARED from '@ducat-unit/core/schema';
export function borrow_vault_api(client) {
    return (request) => {
        validate_vault_borrow_request(request);
        const msg = { data: request, topic: TOPICS.GUARDIAN.VAULT_BORROW };
        ...
        emit_info(observe, 'guardian.request.vault.borrow.start', 'created guardian vault borrow request', { contract_id: request.contract_id });
        sub.on('message', (message) => {
            switch (message.type) {
                ...
                case 'reject': {
                    const parsed = SHARED.base.str.parse(message.data);
                    emit_warn(observe, 'guardian.request.vault.borrow.reject', 'guardian vault borrow request rejected', {
                        reason: parsed
                    });
                ...
                case 'result': {
                    const schema = SCHEMA.module.guard.response.vault_borrow;
                    const parsed = schema.parse(message.data);
                    emit_debug(observe, 'guardian.request.vault.borrow.result', {
                        issue_txid: parsed.issue_txid,
