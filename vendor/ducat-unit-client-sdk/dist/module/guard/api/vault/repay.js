...
import { validate_vault_repay_request } from '../../../../module/vault/lib/validate.js';
import * as SCHEMA from '../../../../schema/index.js';
import * as SHARED from '@ducat-unit/core/schema';
export function repay_vault_api(client) {
    return (request) => {
        validate_vault_repay_request(request);
        const msg = { data: request, topic: TOPICS.GUARDIAN.VAULT_REPAY };
        ...
        emit_info(observe, 'guardian.request.vault.repay.start', 'created guardian vault repay request', { contract_id: request.contract_id });
        sub.on('message', (message) => {
            switch (message.type) {
                ...
                case 'reject': {
                    const parsed = SHARED.base.str.parse(message.data);
                    emit_warn(observe, 'guardian.request.vault.repay.reject', 'guardian vault repay request rejected', {
                        reason: parsed
                    });
                ...
                case 'result': {
                    const schema = SCHEMA.module.guard.response.vault_repay;
                    const parsed = schema.parse(message.data);
                    emit_debug(observe, 'guardian.request.vault.repay.result', {
                        burn_txid: parsed.burn_txid,
