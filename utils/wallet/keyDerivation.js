 ...
 * @param {string} address - Address (tb1p... or tb1q...)
...
export async function getPrivateKeyForAddress(address) {
  ...
  try {
    ...
    logger.warn('[getPrivateKeyForAddress] Failed to read cache:', error.message);
  }
  ...
  try {
    ...
    logger.warn('[getPrivateKeyForAddress] Failed to cache:', error.message);
  }
 ...
 */
function findTaprootAccount(child, address, accountIndex, derivationPath) {
 ...
 */
function findSegwitAccount(child, address, accountIndex, derivationPath) {
