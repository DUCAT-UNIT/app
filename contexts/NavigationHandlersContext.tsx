...
import { useWallet, type WalletAccountSwitchOptions } from './WalletContext';
...
export interface AccountSwitcherContextValue {
  ...
  setNewWalletProfile: React.Dispatch<React.SetStateAction<WalletImportProfile>>;
  switchingAccount: boolean;
  switchAccount: (index: number, options?: WalletAccountSwitchOptions) => Promise<void>;
}
...
export const NavigationHandlersProvider: React.FC<NavigationHandlersProviderProps> = ({
  children,
  walletExists,
}) => {
  ...
  const { setIsAuthenticated, setBiometricEnabled, biometricEnabled, passkeyEnabled, resetAuth } =
    useAuthSession();
  const {
    setSettingUpPin,
    setChangingPin,
  ...
  const { resetWallet, switchAccount: switchAccountContext, walletProfile } = useWallet();
  ...
  const { setSeedConfirmed, resetWalletAndState: onboardingResetWalletAndState } =
    useOnboardingFlow();
  ...
  const {
    ...
    currentWalletProfile: walletProfile,
    resetBalances,
    fetchBalance,
  ...
  return (
    <SettingsHandlersContext.Provider value={settingsValue}>
      <AccountSwitcherCtx.Provider value={accountSwitcherValue}>
        <AuthFlowCtx.Provider value={authFlowValue}>{children}</AuthFlowCtx.Provider>
