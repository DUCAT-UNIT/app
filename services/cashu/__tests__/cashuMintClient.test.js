...
describe('cashuMintClient', () => {
  ...
  describe('getMintInfo', () => {
    it('should fetch mint info', async () => {
      ...
      getJSON.mockResolvedValue(mockInfo);
  ...
  describe('getKeysets', () => {
    it('should fetch keysets', async () => {
      ...
      getJSON.mockResolvedValue(mockKeysets);
  ...
  describe('getKeys', () => {
    it('should fetch all keys when no keyset ID provided', async () => {
      ...
      getJSON.mockResolvedValue(mockKeys);
  ...
  describe('createMintQuote', () => {
    it('should create a mint quote', async () => {
      ...
      postJSON.mockResolvedValue(mockQuote);
    ...
    it('should throw on error', async () => {
      postJSON.mockRejectedValue(new Error('Server error'));
  ...
  describe('checkMintQuote', () => {
    it('should check mint quote status', async () => {
      ...
      getJSON.mockResolvedValue(mockQuote);
    ...
    it('should throw on error', async () => {
      getJSON.mockRejectedValue(new Error('Quote not found'));
  ...
  describe('mintTokens', () => {
    ...
    it('should throw on mint error response', async () => {
      postJSON.mockResolvedValue({ error: 'Quote expired' });
    ...
    it('should throw on invalid signatures format', async () => {
      postJSON.mockResolvedValue({ signatures: 'not-an-array' });

      await expect(mintTokens('quote123', [])).rejects.toThrow('Invalid mint response: missing signatures');
    });

    it('should throw on network error', async () => {
      postJSON.mockRejectedValue(new Error('Network timeout'));
  ...
  describe('swapTokens', () => {
    it('should swap tokens successfully', async () => {
      ...
      const inputs = [{ amount: 100, secret: 's', C: 'c' }];
    ...
    it('should throw on swap error response', async () => {
      postJSON.mockResolvedValue({ error: 'Proofs already spent' });
  ...
  describe('createMeltQuote', () => {
    ...
    it('should throw on error', async () => {
      postJSON.mockRejectedValue(new Error('Invalid address'));
  ...
  describe('checkProofsSpent', () => {
    ...
    it('should throw when dynamic import fails', async () => {
      ...
      await expect(checkProofsSpent([{ secret: 's' }])).rejects.toThrow();
    });
  });
});
