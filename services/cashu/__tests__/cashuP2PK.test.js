...
describe('cashuP2PK', () => {
  beforeEach(() => {
    ...
    ecc.signSchnorr.mockReset();
    ecc.signSchnorr.mockReturnValue(new Uint8Array(64).fill(0xcd));
  });
  ...
  describe('generateP2PKKeyPair', () => {
    it('should generate a keypair with privateKey and publicKey', async () => {
      ...
      schnorr.getPublicKey.mockReturnValue(new Uint8Array(32).fill(0xcd));
  ...
  describe('createP2PKSecret', () => {
    it('should create a P2PK secret with default options', async () => {
      crypto.getRandomBytesAsync.mockResolvedValue(new Uint8Array(32).fill(0x11));
  ...
  describe('signP2PKSecret', () => {
    ...
    it('should throw enhanced error on signing failure', async () => {
      ecc.signSchnorr.mockImplementationOnce(() => {
    ...
    it('should include diagnostics for null secret', async () => {
      ...
      await expect(signP2PKSecret(null, 'a'.repeat(64))).rejects.toThrow('Secret is null/undefined');
    });
    ...
    it('should include diagnostics for null private key', async () => {
      ...
      await expect(signP2PKSecret('secret', null)).rejects.toThrow('Private key is null/undefined');
    });
    ...
    it('should include diagnostics for non-string private key', async () => {
      ...
      await expect(signP2PKSecret('secret', 12345)).rejects.toThrow('expected string');
    });
  ...
  describe('verifyP2PKWitness', () => {
    it('should return true for valid witness signature', async () => {
      ...
      crypto.digest.mockResolvedValue(new ArrayBuffer(32));
      schnorr.verify.mockReturnValue(true);
    ...
    it('should return false on verification error', async () => {
      const witness = JSON.stringify({ signatures: ['a'.repeat(128)] });
      crypto.digest.mockRejectedValue(new Error('Digest failed'));
    ...
    it('should return false when schnorr.verify returns false', async () => {
      ...
      crypto.digest.mockResolvedValue(new ArrayBuffer(32));
      schnorr.verify.mockReturnValue(false);
  ...
  describe('findAccountForP2PKToken', () => {
    const { withMnemonic, getCurrentAccount } = require('../../secureStorageService.js');
  ...
  describe('getP2PKPrivateKey', () => {
    const { withMnemonic, getCurrentAccount } = require('../../secureStorageService.js');
    const { deriveAddressesFromMnemonic } = require('../../../utils/bitcoin.js');
    const { getPrivateKeyForAddress } = require('../../../utils/wallet');

    beforeEach(() => {
      getCurrentAccount.mockResolvedValue(0);
      deriveAddressesFromMnemonic.mockResolvedValue({
        taprootAddress: 'tb1ptest123',
        segwitAddress: 'tb1qtest123',
      });
      getPrivateKeyForAddress.mockResolvedValue({
        privateKey: 'derived_private_key_hex',
        xOnlyPubkey: 'derived_pubkey_hex',
        accountIndex: 0,
      });
    ...
    it('should handle cache read error gracefully', async () => {
      SecureStore.getItemAsync.mockRejectedValue(new Error('Read failed'));
