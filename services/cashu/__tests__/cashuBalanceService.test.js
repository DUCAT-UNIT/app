...
describe('cashuBalanceService', () => {
  ...
  describe('getOrFetchKeys', () => {
    it('should fetch fresh keys when cache is empty', async () => {
      ...
      getKeys.mockResolvedValue(mockKeysetData);
    ...
    it('should handle old cache format without timestamp (line 34)', async () => {
      ...
      SecureStore.getItemAsync.mockResolvedValue(JSON.stringify(oldFormatCache));
    ...
    it('should throw and log error on fetch failure (line 51-52)', async () => {
      const { logger } = require('../../../utils/logger');
      getKeys.mockRejectedValue(new Error('Network error'));
  ...
  describe('getBalance', () => {
    ...
    it('should filter out P2PK locked proofs', async () => {
      loadProofs.mockResolvedValue([
        { amount: 64, secret: 'normal' },
        { amount: 32, secret: '[\"P2PK\",{}]' },
      ]);
      isP2PKSecret.mockImplementation(secret => secret.startsWith('[\"P2PK\"'));
      sumProofs.mockReturnValue(0.64);
    ...
    it('should use partial load when fullLoad is false', async () => {
      loadProofsPartial.mockResolvedValue([{ amount: 64, secret: 's1' }]);
      sumProofs.mockReturnValue(0.64);
