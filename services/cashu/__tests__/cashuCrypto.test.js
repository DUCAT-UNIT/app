...
describe('cashuCrypto', () => {
  ...
  describe('generateBlindingFactor', () => {
    it('should generate a 64 character hex string', async () => {
      crypto.getRandomBytesAsync.mockResolvedValue(new Uint8Array(32).fill(0xcd));
  ...
  describe('sumProofs', () => {
    ...
    it('should handle single proof', () => {
      const sum = sumProofs([{ amount: 500 }]);
  ...
  describe('hashToCurve', () => {
    ...
    it('should throw error if no valid point found', async () => {
      ...
      Point.fromHex.mockImplementation(() => {
        throw new Error('Invalid point');
      });
  ...
  describe('createBlindedMessage', () => {
    beforeEach(() => {
      ...
      Point.fromHex.mockImplementation((hex) => {
        if (hex.startsWith('02')) return { x: hex };
        throw new Error('Invalid point');
      });
    });
    ...
    it('should throw on blinding failure', async () => {
      ...
      ecc.pointFromScalar.mockImplementationOnce(() => {
  ...
  describe('unblindSignature', () => {
    beforeEach(() => {
      ecc.pointMultiply.mockReturnValue(new Uint8Array([0x02, ...new Array(32).fill(0)]));
      ecc.pointAdd.mockReturnValue(new Uint8Array(33).fill(0xab));
    });
    ...
    it('should negate y-coordinate by flipping prefix 03 to 02', () => {
      ...
      ecc.pointMultiply.mockReturnValue(new Uint8Array([0x03, ...new Array(32).fill(0)]));
    ...
    it('should return C_ on error (fallback)', () => {
      ecc.pointMultiply.mockImplementationOnce(() => {
  ...
  describe('selectProofsForAmount - greedy algorithm', () => {
    it('should use greedy algorithm for large number of proofs (>15)', () => {
      ...
      const proofs = Array.from({ length: 20 }, (_, i) => ({ amount: i + 1 }));
