...
interface UseQRScannerReturn {
  ...
  progress: number;
  isScanning: boolean | number;
  totalChunks: number | null;
  scannedChunks: Map<number, string>;
  bcurProgress: number;
}

export function useQRScanner({ visible, onScan }: UseQRScannerParams): UseQRScannerReturn {
  const [scannedChunks, setScannedChunks] = useState<Map<number, string>>(new Map());
  const [totalChunks, setTotalChunks] = useState<number | null>(null);
  const [bcurDecoder, setBcurDecoder] = useState<URDecoder | null>(null);
  const [bcurProgress, setBcurProgress] = useState<number>(0);
  const [, setBcurReceivedParts] = useState<number>(0);
  const [bcurExpectedParts, setBcurExpectedParts] = useState<number | null>(null);
  ...
  const handleNut16Scan = useCallback((match: RegExpMatchArray) => {
