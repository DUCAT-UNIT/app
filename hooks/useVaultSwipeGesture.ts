...
type TabType = 'wallet' | 'vault';

interface UseVaultSwipeGestureParams {
  activeTab: TabType;
  setActiveTab: (tab: TabType) => void;
  openVault: () => void;
}

interface UseVaultSwipeGestureReturn {
  vaultTranslateX: Animated.Value;
  walletTranslateX: Animated.Value;
  isSwiping: boolean;
  walletPanResponder: PanResponderInstance;
  vaultPanResponder: PanResponderInstance;
}

/**
 * Hook for managing swipe gestures between wallet and vault screens
 */
export function useVaultSwipeGesture({
  activeTab,
  setActiveTab,
  openVault
}: UseVaultSwipeGestureParams): UseVaultSwipeGestureReturn {
